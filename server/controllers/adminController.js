...
export const createStudent = asyncHandler(async (req, res) => {
  ...
  const welcomeMessage = autoGenerated
    ? `Hello ${name},<br><br>An administrator created your Pragya Yoga account.<br><strong>Email:</strong> ${student.email}<br><strong>Temporary Password:</strong> ${raw}<br><br>Please log in and change your password.`
  ...
  notificationService.send(student._id, {
    ...
    subject: autoGenerated ? 'Your Pragya Yoga Account Credentials' : `Welcome to Pragya Yoga, ${student.name}!`,
...
export const assignPlan = asyncHandler(async (req, res) => {
  ...
  await log(`Recorded manual payment ₹${assignedPrice} for plan assignment`, req, studentId, { planType: planType || plan.name, amount: assignedPrice });
  ...
  await Promise.all([
    ...
    notificationService.send(studentId, {
      ...
      data: {
        planName: m.planType,
        amount: `₹${(price || m.price || 0).toLocaleString('en-IN')}`,
...
export const renewMembership = asyncHandler(async (req, res) => {
  ...
  notificationService.send(studentId, {
    ...
    data: {
      planName: plan.name,
      amount: `₹${(plan.price || 0).toLocaleString('en-IN')}`,
      invoiceDate: new Date().toLocaleDateString('en-IN'),
      name: student.name,
...
export const createPayment = asyncHandler(async (req, res) => {
  ...
  await log(`Recorded manual payment ₹${amount} for ${user}`, req, user, { paymentId: p._id });
...
export const plans = crud(Plan, 'plan', [
  'name', 'price', 'durationMonths', 'benefits', 'features',
  'membershipAccess', 'pauseDays', 'active', 'displayOrder', 'isPopular',
]);
...
export const updateConsultation = asyncHandler(async (req, res) => {
  ...
  if (c.status === 'completed' && c.user && c.paymentRef) {
    await notify(c.user, { title: 'Consultation completed', message: `Your consultation on ${c.date?.toLocaleDateString('en-IN')} has been marked as completed.`, type: 'success' });
  }
