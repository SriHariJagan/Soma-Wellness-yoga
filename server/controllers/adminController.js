...
export const getOverview = asyncHandler(async (req, res) => {
  ...
  const [
    ...
    User.countDocuments({ role: 'student', status: 'active', isDeleted: { $ne: true } }),
    User.countDocuments({ role: 'student', status: 'banned', isDeleted: { $ne: true } }),
    User.countDocuments({ role: 'student', isDeleted: { $ne: true }, createdAt: { $gte: startOfMonth } }),
    Lead.countDocuments(),
    Batch.countDocuments(),
    Booking.countDocuments(),
    Booking.countDocuments({ status: 'Pending' }),
    Payment.aggregate([{ $match: { paymentStatus: 'captured' } }, { $group: { _id: null, total: { $sum: '$amount' } } }]),
    User.find({ role: 'student', isDeleted: { $ne: true } }).sort({ createdAt: -1 }).limit(6).select('name email createdAt status planMonths'),
...
export const createStudent = asyncHandler(async (req, res) => {
  ...
  const welcomeMessage = autoGenerated
    ? `Hello ${name},<br><br>An administrator created your SomaWellness account.<br><strong>Email:</strong> ${student.email}<br><strong>Temporary Password:</strong> ${raw}<br><br>Please log in and change your password.`
    : `Hello ${name},<br><br>Welcome to SomaWellness! Your account is ready — sign in with the password you chose.`;
  ...
  notificationService.send(student._id, {
    ...
    subject: autoGenerated ? 'Your SomaWellness Account Credentials' : `Welcome to SomaWellness, ${student.name}!`,
...
export const assignPlan = asyncHandler(async (req, res) => {
  ...
  await syncDailyWithMembership(studentId).catch(() => {});
  res.json(m);
});
