...
import {
  yttcInvitesApi, getBatches, servicesApi, coursesApi, workshopsApi,
...
const RECIPIENT_OPTIONS = [
  { value: 'custom', label: 'Active YTTC Students', icon: <LuGraduationCap size={14} /> },
];
...
function DatePicker({ value, onChange, label, error, required }) {
  ...
  const formatted = value ? formatDisplayDate(value) : '';
    ...
    >
      ...
      >
        ...
        <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
          ...
          >
            {loading ? <><LuLoader size={14} style={{ animation: 'spin 0.8s linear infinite' }} /> Sending…</> : confirmText || 'Send YTTC Invitation'}
          </button>
        </div>
      </div>
...
/* ═══════════════════════════════════════════════════════════════
   MAIN COMPONENT
   ═══════════════════════════════════════════════════════════════ */
export default function YTTCInvites() {
  ...
  const loadInvites = useCallback(async () => {
    try {
      ...
      const [d, st] = await Promise.all([
        yttcInvitesApi.list(params),
        yttcInvitesApi.stats().catch(() => null),
      ]);
  ...
  const fetchRecipients = useCallback(async (type) => {
    ...
    try {
      ...
      if (type === 'service_members' && form.serviceId) params.serviceId = form.serviceId;
      ...
      const data = await yttcInvitesApi.recipients(params);
  ...
  }, [form.batchId, form.serviceId, form.courseId, form.workshopId, flash]);
  ...
  const handleCustomSearch = useCallback(async (query) => {
    ...
    setLoadingRecipients(true);
    try {
      const data = await yttcInvitesApi.recipients({ type: 'custom', search: query });
  ...
  }, [form.recipientType, selectedIds.length, recipientCandidates.length]);
  ...
  const validationErrors = useMemo(() => {
    ...
    if (recipientCandidates.length === 0 && form.recipientType && form.recipientType !== 'custom') errors.push('No recipients found for the selected criteria');
    return errors;
  }, [form.title, form.date, form.startTime, form.recipientType, selectedIds.length, recipientCandidates.length]);
  ...
  async function handleCreate() {
    ...
    try {
      const payload = {
        ...
        studentIds: form.recipientType === 'custom' ? selectedIds : undefined,
      };
      const result = await yttcInvitesApi.create(payload);
      flash(`YTTC invitation sent to ${result.invite?.totalRecipients || 0} students.`);
      setForm(EMPTY_FORM);
      setSelectedIds([]);
      setRecipientCandidates([]);
      setRecipientSearch('');
  ...
  function saveDraft() {
    try {
      localStorage.setItem('yttcInviteDraft', JSON.stringify({ form, selectedIds, recipientType: form.recipientType }));
  ...
  function restoreDraft() {
    try {
      const raw = localStorage.getItem('yttcInviteDraft');
      if (raw) {
  ...
  if (loading && invites.length === 0) {
    return (
      <div>
        <PageHeader title="YTTC Invitations" subtitle="Send and manage YTTC class invitations for students." />
  ...
  return (
    <div>
      ...
      <ConfirmModal
        ...
        title="Send YTTC Invitation?"
        message={`This invitation will be sent to ${getRecipientCount() || 0} student(s). They will receive a notification with all the YTTC class details.`}
        confirmText="Send YTTC Invitation"
        confirmIcon={<LuSend size={24} />}
        loading={saving}
      />
...
function Overview({ invites, stats, loading, search, onSearchChange, statusFilter, onStatusFilterChange, onRefresh, onCreate, onOpenDetail, flash }) {
  return (
    <>
      <PageHeader
        title="YTTC Invitations"
        subtitle="Send and manage YTTC class invitations for students."
      >
        <div className={s.pageHeaderActions}>
          ...
          <button className={`${s.btnPrimary} ${s.btnSm}`} onClick={onCreate}>
            <LuPlus size={14} /> New YTTC Invitation
          </button>
        </div>
      </PageHeader>
      ...
      {stats && (
        <div className={s.statsGrid}>
          ...
          <KpiCard icon={<LuCalendarDays />} accent="amber" label="Upcoming YTTC Classes" value={stats.upcoming || 0} />
        </div>
      )}
                ...
                <tr>
                  <td colSpan={9}>
                    <div className={s.emptyState}>
                      ...
                      No YTTC invitations sent yet.
                      <div style={{ marginTop: 8 }}>
                        <button className={`${s.btnPrimary} ${s.btnSm}`} onClick={onCreate}>
                          <LuPlus size={13} /> Create your first YTTC invitation
                        </button>
                      </div>
                    </div>
                  </td>
  ...
  return (
    <>
      <div style={{
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
            ...
            >
              <LuChevronLeft size={14} /> Back to YTTC Invitations
            </button>
          </div>
          ...
          <p style={{ fontSize: 13, color: C.text2, marginTop: 4 }}>Create and send a YTTC class invitation to enrolled students.</p>
        </div>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <button
            ...
            disabled={!canSubmit}
          >
            <LuSend size={13} /> Send YTTC Invitation
          </button>
        </div>
      </div>
      ...
      <div className={s.gridDash}>
        {/* ── LEFT COLUMN ── */}
        <div>
          <CollapsibleCard title="YTTC Class Details" icon={<LuCalendar size={17} />}>
            <FloatingInput label="YTTC Class Title" icon={<LuMail size={15} />} value={form.title} onChange={e => update({ title: e.target.value })} required helperText="e.g. 200 Hr YTTC Live Session" />
            <div style={{ height: 8 }} />
            ...
            <div style={{ height: 8 }} />
            <FloatingTextarea label="Description" icon={<LuFileText size={15} />} value={form.description} onChange={e => update({ description: e.target.value })} helperText="Brief description of the YTTC class (optional)" />
          </CollapsibleCard>
        ...
        <div style={{ position: 'sticky', top: 90 }}>
          <StickyCard title="Recipients" icon={<LuUsers size={16} />} accent>
            ...
            {form.recipientType === 'service_members' && !loadingRecipients && recipientCandidates.length > 0 && (
              <div style={{
                marginTop: 8, padding: '6px 10px', background: C.primarySoft,
                borderRadius: 8, fontSize: 12, color: C.primary, fontWeight: 600,
              }}>
          ...
          <StickyCard title="Invitation Summary" icon={<LuClipboardList size={16} />}>
              ...
              <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                ...
                {form.startTime && (
                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 11.5, color: C.text2 }}>
                    ...
                    {formatTimeDisplay(form.startTime)}{form.endTime && form.endTime !== 'undefined' ? ` - ${formatTimeDisplay(form.endTime)}` : ''}
                  </span>
                )}
              </div>
              {form.platform && (
            ...
            >
              <LuSend size={15} /> Send YTTC Invitation
            </button>
          </StickyCard>
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 5, fontSize: 12, color: C.text2 }}>
                ...
                {form.startTime && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <LuClock size={12} style={{ color: C.primary }} />
                    <span>{formatTimeDisplay(form.startTime)}{form.endTime && form.endTime !== 'undefined' ? ` - ${formatTimeDisplay(form.endTime)}` : ''}</span>
                  </div>
                )}
                {form.duration && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
...
function DetailView({ inviteId, onBack, onRefresh, flash }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await yttcInvitesApi.get(inviteId);
  ...
  if (loading) {
    return (
      <div>
        <PageHeader title="YTTC Invitation Details" subtitle="Loading..." />
        <div className={s.card}>
  ...
  if (!detail) {
    return (
      <div>
        <PageHeader title="YTTC Invitation Details" subtitle="Not found">
  ...
  const handleCancel = async () => {
    ...
    try {
      await yttcInvitesApi.cancel(inviteId, reason || '');
  ...
  const handleResend = async () => {
    try {
      const result = await yttcInvitesApi.resend(inviteId);
  ...
  const handleDuplicate = async () => {
    try {
      await yttcInvitesApi.duplicate(inviteId);
